...
contract Airdrop {
    ...
    function changeState(uint8 newState) public isOwner returns(status) {
    ...
    function hasSubscribed(address subscriber) private view returns(bool) {
        for (uint256 i = 0; i < subscribers.length; i++) {
            require(subscriber != subscribers[i], "This wallet is already subscribed!");
        }
